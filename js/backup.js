 ...
 * 個人情報保護厳守: 個人情報（患者氏名・生年月日・処方等）の非保持スキャンをインポート時に徹底
 */
...
class BackupManager {
  ...
  /**
   * 個人情報非保持ディープスキャン
   * プライバシー保護ルール: 患者氏名（漢字・ひらがな・カタカナ等）が返戻カルテ番号等に含まれていないか検査
   */
  ...
  static validateBackupData(input) {
    ...
    if (!scanResult.safe) {
      return {
        valid: false,
        error: '【個人情報保護ルール】ファイル内に患者氏名等の個人情報が検出されたため、安全のため復元を中止しました。',
