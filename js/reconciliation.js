 ...
 * 開発憲法厳守: 個人情報（氏名・処方等）は一切保持せず、カルテ番号のみで管理する。
 */
...
class ReconciliationManager {
  ...
  static validatePatientChartId(chartId) {
    ...
    // 日本語（漢字・ひらがな・カタカナ）が含まれている場合は個人情報漏洩防止のためエラー
    const containsJapanese = /[\u3000-\u303F\u3040-\u309F\u30A0-\u30FF\uFF00-\uFFEF\u4E00-\u9FAF]/;
    if (containsJapanese.test(trimmed)) {
      throw new Error('【開発憲法・個人情報保護規約】患者氏名・個人情報は入力できません。カルテ番号（半角英数・記号）のみを入力してください。');
    }
    return trimmed;
  }
  ...
  loadPhase4ConstitutionalTestData() {
    ...
    this.addRemandItem({
      ...
      handlingNote: '【憲法検証】7/12受診時保険証失効。患者へ新保険証提出を依頼中'
    });
  }
